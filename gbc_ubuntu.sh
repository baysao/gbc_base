...
_env() {
	sudo apt install -y python3-virtualenv python3-pip python3 supervisor
...
_lualib() {
	...
	sudo mkdir -p /usr/local/openresty/luajit/lib/lua/5.1/socket/
	...
	make -j$(nproc) && sudo make install-unix
	...
	sudo cp -f process.so /usr/local/openresty/luajit/lib/lua/5.1
}
_lib_() {
	# install luabson
	echo ""
...
_beanstalk() {
	...
	make -j$(nproc)
	sudo cp -f beanstalkd /usr/local/bin
}
# _tools() {
# 	mkdir -p $DEST_BIN_DIR/bin
